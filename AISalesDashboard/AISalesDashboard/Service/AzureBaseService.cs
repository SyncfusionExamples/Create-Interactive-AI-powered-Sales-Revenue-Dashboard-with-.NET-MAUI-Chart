...
namespace AISalesDashboard
{
    public class AzureBaseService
    {
        ...
        internal async Task<string> GetAnswerFromGPT(string userPrompt)
        {
            ...
            {
                return "currenty in offline mode";
            }
       ...
       DateRange predictionPeriod)
        ...
        public List<SalesPrediction> GetPredictionsFromEmbeddedJson(DateTime startDate)
        {
            ...
            using (var stream = executingAssembly.GetManifestResourceStream("AISalesDashboard.Maui.AISalesDashboard.Resources.Raw.prediction.json"))
            using (var textStream = new StreamReader(stream!))
            {
                // Read the JSON content from the embedded resource
                string json = textStream.ReadToEnd();

                var currentpredictionData = JsonSerializer.Deserialize<List<SalesPrediction>>(json);

                for (int i = 0; i < currentpredictionData!.Count; i++)
                {
                    currentpredictionData[i].Date = startDate.AddDays(i);
                }

                return currentpredictionData ?? new List<SalesPrediction>();
            }
        }
